...
def campaign_detail(campaign_id):
    ...
    try:
        ...
        if campaign.get('created_at'):
            ...
            campaign['days_active'] = days_active
        else:
            campaign['days_active'] = 1
        ...
        if campaign_id in ['1', '2', '3']:
            ...
            for donation in recent_donations:
                if not donation.get('is_anonymous', False):
                    donor = get_user_by_id(donation['donor_id'])
                    donation['donor'] = donor if donor else {'username': 'Unknown User'}
                else:
        ...
        print(f"Error in campaign_detail route: {e}")
        import traceback
        traceback.print_exc()
        return "Campaign not found or invalid ID", 404
...
def make_donation():
    ...
    if not campaign.get('is_active', True):
        return jsonify({'error': 'Campaign is not currently accepting donations'}), 400
...
@app.route('/api/admin/delete_campaign/<campaign_id>', methods=['DELETE'])
def delete_campaign(campaign_id):
    """Admin route to delete a campaign"""
    if 'user_id' not in session:
        return jsonify({'error': 'Authentication required'}), 401
    ...
    try:
        ...
        # Delete all donations associated with this campaign
        donations.delete_many({'campaign_id': ObjectId(campaign_id)})
